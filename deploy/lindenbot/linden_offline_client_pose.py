...
output_file = "/wx-mix01/sppro/permanent/wjxu22/codes/iFlyBotVLA/debug/sppro_vla_pose_check_1.json"

pose_fields = ["l_s_p_x", "l_s_p_y", "l_s_p_z", "l_s_o_w", "l_s_o_x", "l_s_o_y", "l_s_o_z",  
               "r_s_p_x", "r_s_p_y", "r_s_p_z", "r_s_o_w", "r_s_o_x", "r_s_o_y", "r_s_o_z"]
action_fields = ["l_s_0", "l_s_1", "l_s_2", "l_s_3", "l_s_4", "l_s_5", "l_s_6", "l_s_7",
                 "r_s_0", "r_s_1", "r_s_2", "r_s_3", "r_s_4", "r_s_5", "r_s_6", "r_s_7"]


axis_names = ['x', 'y', 'z', 'qw', 'qx', 'qy', 'qz', 'grip']
def paint(all_actions, all_labels, timesteps, eps_idx):
    ...
    plt.tight_layout()
    plt.savefig(f"outputs/left_arm_pose_eps{eps_idx}.png", dpi=200)
    plt.show()
    plt.close(fig)
...
def eval_video(args: Args) -> None:
    ...
    old_csv_path = episode_dir / "output_fk_ee.csv"
    csv_path = episode_dir / "pose.csv"
    ...
    actions = old_df[action_fields].values.astype(np.float32)
    actions = actions[:, :act_dim]
    poses = df[pose_fields].values.astype(np.float32)

    states = np.zeros((poses.shape[0], act_dim))
    states[:, 0:7] = poses[:, 0:7]
    states[:, 7] = actions[:, 7]

    if is_bimanual:
        states[:, 8:15] = poses[:, 7:14]
