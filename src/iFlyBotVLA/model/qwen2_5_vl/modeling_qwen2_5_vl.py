...
class Qwen2_5_VLForConditionalGenerationSTATE(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.domain_aware = getattr(config, "domain_aware", False)
        if self.domain_aware:
            self.proprios_embed = DomainAwareLinear(20, config.hidden_size)
            self.soft_prompt_hub = nn.Embedding(30, 32 * config.hidden_size)
            nn.init.normal_(self.soft_prompt_hub.weight, std=0.02)
        else:
            self.proprios_embed = nn.Linear(20, config.hidden_size)
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if proprios is not None:
                ...
                mask = input_ids == STATE_PAD_TOKEN_INDEX
                mask_unsqueezed = mask.unsqueeze(-1)
                mask_expanded = mask_unsqueezed.expand_as(inputs_embeds)
                proprios_mask = mask_expanded.to(inputs_embeds.device)
                if self.domain_aware:
                    proprios_embeds = self.proprios_embed(proprios, domain_id).to(inputs_embeds.device, inputs_embeds.dtype)
                else:
                    proprios_embeds = self.proprios_embed(proprios).to(inputs_embeds.device, inputs_embeds.dtype)
            ...
            if domain_id is not None:
                # domain_id = domain_id.type(self.visual.dtype)
                mask = input_ids == DOMAIN_PAD_TOKEN_INDEX
                mask_unsqueezed = mask.unsqueeze(-1)
                mask_expanded = mask_unsqueezed.expand_as(inputs_embeds)
                domain_mask = mask_expanded.to(inputs_embeds.device)
                domain_embeds = self.soft_prompt_hub(domain_id).view(-1, 32, self.hidden_size).to(inputs_embeds.device, inputs_embeds.dtype)
                inputs_embeds = inputs_embeds.to(inputs_embeds.device, inputs_embeds.dtype)
                inputs_embeds = inputs_embeds.masked_scatter(domain_mask, domain_embeds)
...
__all__ = ["Qwen2_5_VLForConditionalGenerationSTATE", "Qwen2_5_VLModel", "Qwen2_5_VLPreTrainedModel"]
