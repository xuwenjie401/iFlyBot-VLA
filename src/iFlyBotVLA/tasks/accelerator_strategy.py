...
class AcceleratorStrategy(ABC):
    def __init__(
        ...
        self.special_training_stage = special_training_stage
    ...
    def run_setup(self, n_train_examples: int) -> None:
        ...
        if self.special_training_stage == "pretrain_stage_2":
            """
            Stage-2 预训练策略：
              - VLM: lr 从 0 warmup 到 (self.learning_rate / 10)
              - action_head: lr 恒定为 self.learning_rate
              - 沿用原先的 decay/no_decay 规则  bias / LayerNorm 等不做 weight decay
            """
            ...
            # -------------------------
            # 1) 按模块拆分参数：VLM vs action_head
            # -------------------------
            vlm_decay, vlm_no_decay = [], []
            act_decay, act_no_decay = [], []
            # debug
            # act_decay_names, act_no_decay_names = [], []
            # vlm_decay_names, vlm_no_decay_names = [], []

            for name, param in self.vla.named_parameters():
                if not param.requires_grad:
                    continue

                # 拿到 action_head 的参数集合（用 id）
                action_head_param_ids = {
                    id(p) for p in self.vla.action_expert.parameters()
                }
                is_action_head = id(param) in action_head_param_ids

                # 原规则：ndim <= 1 或 bias 不做 weight_decay
                is_no_decay = (param.ndim <= 1) or name.endswith(".bias")

                if is_action_head:
                    if is_no_decay:
                        act_no_decay.append(param)
                        # act_no_decay_names.append(name)
                    else:
                        act_decay.append(param)
                        # act_decay_names.append(name)
                else:
                    # 其它都算 VLM（包含 vlm / planning heads 等）
                    if is_no_decay:
                        vlm_no_decay.append(param)
                        # vlm_no_decay_names.append(name)
                    else:
                        vlm_decay.append(param)
                        # vlm_decay_names.append(name)
            ...
            #    - action_head lr = lr
            # -------------------------
            vlm_lr = self.learning_rate / 10.0
            act_lr = self.learning_rate

            groups = [
                {"params": vlm_decay,    "weight_decay": self.weight_decay, "lr": vlm_lr},
                {"params": vlm_no_decay, "weight_decay": 0.0,               "lr": vlm_lr},
                {"params": act_decay,    "weight_decay": self.weight_decay, "lr": act_lr},
                {"params": act_no_decay, "weight_decay": 0.0,               "lr": act_lr},
            ]
            ...
            # -------------------------
            # 3) Scheduler：只 warmup VLM，不动 action_head
            # -------------------------
            if num_warmup_steps <= 0:
                # 没有 warmup：VLM 直接用 base lr=lr/10
                vlm_lambda = lambda step: 1.0
            else:
                # 线性 warmup：step 从 0..warmup -> 0..1
                def vlm_lambda(step: int):
                    # LambdaLR 的 step 从 0 开始增长，这里做安全处理
                    return min(1.0, float(step) / float(num_warmup_steps))

            # action_head 恒定
            act_lambda = lambda step: 1.0

            self.lr_scheduler = LambdaLR(
                self.optimizer,
                lr_lambda=[vlm_lambda, vlm_lambda, act_lambda, act_lambda]
            )

            # -------------------------
            # 4) 让 VLM 从 0 开始：把 VLM 两组的 lr 先手动置 0
            #    action_head 保持 act_lr 不变
            # -------------------------
            for i, pg in enumerate(self.optimizer.param_groups):
                if i in (0, 1):  # 前两组是 VLM
                    pg["lr"] = 0.0
    ...
    def train_step(
        ...
        if self.accelerator.is_main_process and debug:
            torch.cuda.synchronize()
            print(f'forward time={(time.time()-start_time)*1000}')
            start_time = time.time()
        # Commit Loss (Prior to Gradient Accumulation Normalization)
        action_preds = vlm_output.logits[:,:-1].argmax(dim=2)
        ...
        if self.accelerator.is_main_process and debug:
            torch.cuda.synchronize()
            print(f'backward time={(time.time()-start_time)*1000}')
            start_time = time.time()

        if self.accelerator.is_main_process:
            print(f"global_step={metrics.global_step}  l1 loss = {action_loss}, cross loss =  {vlm_loss}, action_accuracy = {action_accuracy}")
        ...
        if batch_qa is not None:
            ...
            with torch.autocast(
                ...
                weighted_qa_loss = qa_loss * qa_loss_scale  # 比如 config 里有个 scale

            # 第二次 backward：只为 QA 的 VLM loss 反传
            self.accelerator.backward(weighted_qa_loss)
        else:
            weighted_qa_loss = None

        if self.accelerator.is_main_process and batch_qa is not None:
    ...
    def run_training(
        ...
        if vqa_dataloader is not None:
            ...
            self.vla, self.optimizer, self.lr_scheduler, _ = self.accelerator.prepare(self.vla, self.optimizer, self.lr_scheduler, vla_dataloader)
        ...
        with tqdm(
            ...
            for step, batch_robot in enumerate(vla_dataloader, start=1):
                ...
                if self.accelerator.is_main_process and debug:
                    torch.cuda.synchronize()
                    print(f'io time={(time.time()-start_time)*1000}')
                    start_time = time.time()
                ...
                batch_qa = None
                if vqa_dataloader is not None and step % qa_every_n_steps == 0:
                    batch_qa = next(vqa_iter)

                vla_loss, qa_loss = self.train_step(metrics=metrics, batch_robot=batch_robot, batch_qa=batch_qa, 
                                                    action_loss_weight=action_loss_weight, qa_loss_scale=1.0)
                ...
                if (metrics.global_step + 1) % self.grad_accumulation_steps == 0:
                    ...
                    # print(f"max_steps: {self.max_steps}\n epoch: {epoch}\n metrics-global_step: {metrics.global_step}\n save_step: {save_step}")
                ...
                epoch = (metrics.global_step + 1) // (dataset_length // self.global_batch_size)
                ...
                if (terminate := (self.max_steps is not None and metrics.global_step >= self.max_steps)) or epoch > previous_epoch or metrics.global_step == 1 or (metrics.global_step + 1) % save_step == 0:
                    self.save_checkpoint(
                        metrics.run_dir, metrics.global_step, epoch, self.accelerator.gather(vla_loss).mean().item())
                    dist.barrier()
                    del vla_loss
                    # torch.cuda.empty_cache()
                    # gc.collect()
                ...
                if self.accelerator.is_main_process and debug:
                    torch.cuda.synchronize()
                    print(f'other time={(time.time()-start_time)*1000}')
                    start_time = time.time()
