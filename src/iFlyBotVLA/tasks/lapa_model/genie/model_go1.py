...
from genie.modules import GO1LatentActionModel
import logging
logging.basicConfig(format='%(message)s', level=logging.INFO)



class GO1_LAM(LightningModule):
    """
    ...
    def __init__(
        ...
        super(GO1_LAM, self).__init__()
        assert stage in ['stage-1']

        lam = GO1LatentActionModel
    ...
    def shared_step(self, batch: Dict) -> Tuple:
        ...
        loss_logs = (
            ...
            ("emb_min", outputs["emb"].min()),
            ("emb_max", outputs["emb"].max()),
            ("codebook_min", outputs["z"].min()),
            ("codebook_max", outputs["z"].max()),
