...
class NSVQ_act(torch.nn.Module):
    def __init__(self, dim, num_embeddings, embedding_dim, device=torch.device('cpu'), discarding_threshold=0.1, initialization='normal', code_seq_len=1, patch_size=32, image_size = 256):
        ...
        if code_seq_len == 1:
            ...
            self.cnn_encoder = torch.nn.Sequential(
                torch.nn.Conv2d(in_channels=embedding_dim, out_channels=embedding_dim, kernel_size=3, stride=2, padding=1), 
                torch.nn.ReLU(),
                # torch.nn.Conv2d(in_channels=embedding_dim, out_channels=embedding_dim, kernel_size=(1,3), stride=1, padding=0), 
                torch.nn.AdaptiveAvgPool2d((4, 2)),
            )
    ...
    def decode(self, quantized_input, batch_size):
        ...
        quantized_fdm_input = self.project_out(quantized_input)
        return quantized_fdm_input, quantized_input
    ...
    def forward(self, input_data_first, input_data_last, codebook_training_only=False):
        ...
        quantized_fdm_input, quantized_action_fdm_input = self.decode(quantized_input, batch_size)
        return quantized_fdm_input, quantized_action_fdm_input, perplexity, self.codebooks_used.cpu().numpy(), min_indices.reshape(batch_size, -1)
    ...
    def inference(self, input_data_first, input_data_last, user_action_token_num=None):
        ...
        quantized_fdm_input, quantized_action_fdm_input = self.decode(quantized_input, batch_size)
        return quantized_fdm_input, quantized_action_fdm_input, min_indices.reshape(batch_size, -1)
