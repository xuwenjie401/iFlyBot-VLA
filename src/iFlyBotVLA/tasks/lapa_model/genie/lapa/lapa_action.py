...
from tasks.lapa_model.genie.lapa.attention import Transformer, ContinuousPositionBias
from tasks.lapa_model.genie.lapa.nsvq_act import NSVQ_act
...
class Lapa_action(nn.Module):
    def __init__(
        ...
        *,
        ...
        # self.state_head = nn.Linear(128, 7)  # next_state
        self.action_head = nn.Linear(128, 7) # next_action
    ...
    def forward(
        ...
        video = video.permute(0, 2, 1, 3, 4).contiguous()  # only for rlds dataset
        ...
        *_, h, w, _ = shape
        ...
        tokens, extra_tokens, perplexity, codebook_usage, indices = self.vq(first_tokens, last_tokens, codebook_training_only = False)
        ...
        # import pdb
        # pdb.set_trace()
        flattened_tokens = rearrange(extra_tokens, 'b t d -> b (t d)')
        combined_proprio = torch.cat([flattened_tokens, action[:, 0, :]], dim=-1)
        proprio_features = self.shared_mlp(combined_proprio)
        # state_recon = self.state_head(proprio_features)
        action_recon = self.action_head(proprio_features)

        # proprio_recon_loss = F.mse_loss(state_recon, action[:, 1, :]) + F.mse_loss(action_recon, action[:, 1, :])
        proprio_recon_loss = F.mse_loss(action_recon, action[:, 1, :])

        return recon_loss, proprio_recon_loss, num_unique_indices
    ...
    def inference(
        ...
        *_, h, w, _ = shape
        ...
        if user_action_token_num is not None:
            tokens, extra_tokens, indices = self.vq.inference(first_tokens, last_tokens, user_action_token_num=user_action_token_num)
        else:
            tokens, extra_tokens, indices = self.vq.inference(first_tokens, last_tokens)
        ...
        return returned_recon, indices, action_recon
