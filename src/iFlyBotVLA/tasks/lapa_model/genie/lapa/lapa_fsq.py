...
from tasks.lapa_model.genie.lapa.fsq_adapt_lapa import FSQ
...
class LAPAfsqModel(nn.Module):
    ...
    def forward(
        ...
        *_, h, w, _ = shape
        ...
        tokens, indices = self.vq(first_tokens, last_tokens)
        # import pdb
        # pdb.set_trace()
    ...
    def inference(
        ...
        *_, h, w, _ = shape
        ...
        if user_action_token_num is not None:
            tokens, indices, codes = self.vq.inference(first_tokens, last_tokens, user_action_token_num=user_action_token_num)
        else:
            tokens, indices, codes = self.vq.inference(first_tokens, last_tokens)
        ...
        return returned_recon, indices, tokens
