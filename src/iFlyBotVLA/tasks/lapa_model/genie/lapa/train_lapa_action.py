...
from tasks.lapa_model.genie.lapa.lapa_action import Lapa_action
...
class LAPA_act(LightningModule):
    """
    ...
    def __init__(
        ...
        super(LAPA_act, self).__init__()

        self.lam = Lapa_action(
            dim = 1024,
            quant_dim=32,
            codebook_size = 8,
            image_size = 224,
    ...
    def shared_step(self, batch: Dict) -> Tuple:
        ...
        # import pdb
        # pdb.set_trace()
        recon_loss, proprio_recon_loss, num_unique_indices = self.lam(batch['videos'], batch['action'], step=self.steps)
        self.steps += 1
        loss = recon_loss + proprio_recon_loss
        if torch.isnan(loss):
            self.nan_count += 1
