...
from tasks.latent_action_model.genie.modules.blocks import patchify, unpatchify, SpatioTemporalTransformer, SpatioTransformer, VectorQuantizer, \
...
class ControllableDINOLatentActionModel(nn.Module):
    ...
    def __init__(
        ...
        self.dino_encoder = torch.hub.load(
            repo_or_dir='/wx-mix01/sppro/permanent/wjxu22/model/dinov2-main',  
            model='dinov2_vitb14_reg',           
            source='local',                      
            pretrained=False                   
        )
        missing_keys, unexpected_keys = self.dino_encoder.load_state_dict(torch.load('/wx-mix01/sppro/permanent/wjxu22/model/dinov2_vitb14_reg4_pretrain.pth'))
        self.dino_encoder.requires_grad_(False)
